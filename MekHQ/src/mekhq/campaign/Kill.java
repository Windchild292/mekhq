...
public class Kill implements Serializable {
	...
	public Kill(UUID id, String kill, String killer, LocalDate date) {
		pilotId = id;
		this.killed = kill;
		this.killer = killer;
		this.date = date;
			...
			for (int x = 0; x < nl.getLength(); x++) {
				...
				if (wn2.getNodeName().equalsIgnoreCase("killed")) {
					...
					if(version.getMajorVersion() == 0 && version.getMinorVersion() < 2 && version.getSnapshot() < 14) {
						retVal.oldPilotId = Integer.parseInt(wn2.getTextContent().trim());
					} else {
						retVal.pilotId = UUID.fromString(wn2.getTextContent().trim());
					}
				} else if (wn2.getNodeName().equalsIgnoreCase("killer")) {
					retVal.killer = (wn2.getTextContent().trim());
	...
	public void writeToXml(PrintWriter pw1, int indent) {
	    ...
	    MekHqXmlUtil.writeSimpleXmlTag(pw1, indent + 1, "pilotId", pilotId.toString());
	    MekHqXmlUtil.writeSimpleXmlTag(pw1, indent + 1, "killed", killed);
	    MekHqXmlUtil.writeSimpleXmlTag(pw1, indent + 1, "killer", killer);
