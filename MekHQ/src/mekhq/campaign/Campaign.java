...
public class Campaign implements Serializable, ITechManager {
        ...
        if ((totalPayout.isPositive()) || (null != getRetirementDefectionTracker().getRetirees())) {
            if (getFinances().debit(totalPayout, Transaction.C_SALARY, "Final Payout", getLocalDate())) {
                for (UUID pid : getRetirementDefectionTracker().getRetirees()) {
                    ...
                    if (!getRetirementDefectionTracker().getPayout(pid).getRecruitType().isNone()) {
    ...
    /**
     * @return A new {@link Person} with the dependent primary role.
     */
    ...
    /**
     * Generate a new Person of the given type using whatever randomization options have been given in the
     * CampaignOptions
     *
     ...
     *
     * @param role The primary role
     * @param secondaryRole A secondary role; used for LAM pilots to generate MW + Aero pilot
     * @return A new {@link Person}.
     */
    public Person newPerson(PersonnelRole role, PersonnelRole secondaryRole) {
        return newPerson(role, secondaryRole, getFactionSelector(), getPlanetSelector(), Gender.RANDOMIZE);
    }
     ...
     * @return A new {@link Person}.
     */
    public Person newPerson(PersonnelRole role, PersonnelRole secondaryRole, String factionCode, Gender gender) {
        return newPerson(role, secondaryRole, new DefaultFactionSelector(factionCode), getPlanetSelector(), gender);
    }
     ...
     */
    public Person newPerson(PersonnelRole role, PersonnelRole secondaryRole,
                            AbstractFactionSelector factionSelector,
        ...
        return newPerson(role, secondaryRole, personnelGenerator, gender);
    }
     ...
     */
    public Person newPerson(PersonnelRole role, PersonnelRole secondaryRole,
                            AbstractPersonnelGenerator personnelGenerator, Gender gender) {
        Person person = personnelGenerator.generate(this, role, secondaryRole, gender);

        // Assign a random portrait after we generate a new person
        if (getCampaignOptions().usePortraitForRole(role)) {
     ...
     *
     * @param role The PersonnelRole to search
        ...
        if (getCampaignOptions().useSupportEdge()
            ...
            if ((getCampaignOptions().isDestroyByMargin()
                    ...
                    || (!getCampaignOptions().isDestroyByMargin() && ((tech.getExperienceLevel(false) == SkillType.EXP_ELITE) //if an elite, primary tech and destroy by margin is NOT on
                                    || tech.getPrimaryRole().isVesselCrew()))
    ...
    public int findAtBPartsAvailabilityLevel(IAcquisitionWork acquisition, StringBuilder reportBuilder) {
        ...
        if ((null != contract) && getLocalDate().isBefore(contract.getStartDate())) {
            if (reportBuilder != null) {
                reportBuilder.append(contract.getPartsAvailabilityLevel()).append(" (")
        ...
        if (reportBuilder != null) {
            ...
            if (adminLog != null) {
                reportBuilder.append(adminMod).append("(").append(adminLog.getFullName())
                        .append(", logistics admin)");
            } else {
                reportBuilder.append(adminMod).append("(no logistics admin)");
            }
        }
    ...
    public void hirePersonnelFor(UUID uid, boolean isGM) {
        ...
        while (unit.canTakeMoreDrivers()) {
            ...
            if (unit.getEntity() instanceof LandAirMech) {
                p = newPerson(PersonnelRole.LAM_PILOT);
            } else if (unit.getEntity() instanceof Mech) {
                p = newPerson(PersonnelRole.MECHWARRIOR);
            } else if (unit.getEntity() instanceof SmallCraft
                    || unit.getEntity() instanceof Jumpship) {
                p = newPerson(PersonnelRole.VESSEL_PILOT);
            } else if (unit.getEntity() instanceof ConvFighter) {
                p = newPerson(PersonnelRole.CONVENTIONAL_AIRCRAFT_PILOT);
            } else if (unit.getEntity() instanceof Aero) {
                p = newPerson(PersonnelRole.AEROSPACE_PILOT);
            } else if (unit.getEntity() instanceof Tank) {
                switch (unit.getEntity().getMovementMode()) {
                    case VTOL:
                        p = newPerson(PersonnelRole.VTOL_PILOT);
                        break;
                    case NAVAL:
                    case HYDROFOIL:
                    case SUBMARINE:
                        p = newPerson(PersonnelRole.NAVAL_VEHICLE_DRIVER);
                        break;
                    default:
                        p = newPerson(PersonnelRole.GROUND_VEHICLE_DRIVER);
                }
            } else if (unit.getEntity() instanceof Protomech) {
                p = newPerson(PersonnelRole.PROTOMECH_PILOT);
            } else if (unit.getEntity() instanceof BattleArmor) {
                p = newPerson(PersonnelRole.BATTLE_ARMOUR);
            } else if (unit.getEntity() instanceof Infantry) {
                p = newPerson(PersonnelRole.SOLDIER);
            }
            if (null == p) {
                break;
            }
        ...
        while (unit.canTakeMoreGunners()) {
            ...
            if (unit.getEntity() instanceof Tank) {
                p = newPerson(PersonnelRole.VEHICLE_GUNNER);
            } else if (unit.getEntity() instanceof SmallCraft
                    || unit.getEntity() instanceof Jumpship) {
                p = newPerson(PersonnelRole.VESSEL_GUNNER);
            } else if (unit.getEntity() instanceof Mech) {
                p = newPerson(PersonnelRole.MECHWARRIOR);
            }
        ...
        while (unit.canTakeMoreVesselCrew()) {
            Person p = newPerson(unit.getEntity().isSupportVehicle() ? PersonnelRole.VEHICLE_CREW
        ...
        if (unit.canTakeNavigator()) {
            Person p = newPerson(PersonnelRole.VESSEL_NAVIGATOR);
            if (!recruitPerson(p, isGM)) {
                return;
            }
            unit.setNavigator(p);
        ...
        if (unit.canTakeTechOfficer()) {
            ...
            p = newPerson((unit.getEntity() instanceof Tank) ? PersonnelRole.VEHICLE_GUNNER
                    : PersonnelRole.MECHWARRIOR);
            if (!recruitPerson(p, isGM)) {
                return;
            }
