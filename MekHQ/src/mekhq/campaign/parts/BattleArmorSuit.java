...
public class BattleArmorSuit extends Part {
    ...
    public double getTonnage() {
        ...
        switch (weightClass) {
            case EntityWeightClass.WEIGHT_ULTRA_LIGHT:
                ...
                tons += groundMP * .025;
                if (jumpType.isUMUInfantry()) {
                    tons += jumpMP * .045;
                } else if (jumpType.isVTOL()) {
                    tons += jumpMP * .03;
                } else {
                    tons += jumpMP * .025;
                }
    ...
    public Money getStickerPrice() {
        ...
        switch(weightClass) {
            case EntityWeightClass.WEIGHT_MEDIUM:
                cost = cost.plus(100000).plus(jumpMP * (jumpType.isVTOL() ? 100000 : 75000));
                break;
            case EntityWeightClass.WEIGHT_HEAVY:
                cost = cost.plus(200000).plus(jumpMP * (jumpType.isUMUInfantry() ? 100000 : 150000));
                break;
            case EntityWeightClass.WEIGHT_ASSAULT:
                cost = cost.plus(400000).plus(jumpMP * (jumpType.isUMUInfantry() ? 150000 : 300000));
                break;
            default:
    ...
    protected void loadFieldsFromXmlNode(Node wn) {
        ...
        for (int x = 0; x < nl.getLength(); x++) {
            ...
            if (wn2.getNodeName().equalsIgnoreCase("trooper")) {
                ...
                jumpType = EntityMovementMode.parseFromString(wn2.getTextContent().trim());
